...
class BaitSplitService
{
    ...
    public function rotateCampaignIp(
        ...
        if (
            ...
            && in_array(
                $targetPoolIds[0],
                $this->decoyMonitoredPoolIds($router),
                true
            )
        ) {
            $poolId = $targetPoolIds[0];
    ...
    private function normalizeDecoyDomain(array $domain): array
    {
        ...
        return [
            ...
            'last_group' => [
                ...
                'ip' => (string) ($domain['last_group']['ip'] ?? ''),
                'from' => (int) ($domain['last_group']['from'] ?? 0),
                'to' => (int) ($domain['last_group']['to'] ?? 0),
            ],
        ];
    }
    ...
    private function recordRouterExposure(
        ...
        try {
            ...
            $key = (string) $userId;
            if (isset($campaign['router']['dormant_suspects'][$key])) {
                $isolate = $this->resolveDecoyIsolatePoolId(
                    $campaign['router'] ?? [],
                    ''
                );
                if (
                    $isolate !== ''
                    && in_array($isolate, array_map('strval', $poolIds), true)
                ) {
                    $wakeKey = $this->dormantWakeKey($campaign);
                    Redis::zadd($wakeKey, $now, $key);
                    Redis::expire($wakeKey, 86400 * 30);
                }
            }
        } catch (\Throwable) {
            // 统计失败不能影响订阅。
    ...
    public function wallReport(string $campaignId, int $limit = 100): array
    {
        ...
        $candidatePoolId = $this->resolveDecoyCandidatePoolId($router);
        return [
            'events' => $events,
            ...
            'decoy' => [
                ...
                'monitor_24h' => $this->configBool('decoy_monitor_24h', true),
                'active_date' => (string) ($decoy['active_date'] ?? ''),
                'start' => (string) ($this->config['decoy_start'] ?? '01:00'),
                'end' => (string) ($this->config['decoy_end'] ?? '08:00'),
                'batch_size' => $this->decoyBatchSize(),
                'verify_batch_size' => $this->decoyVerifyBatchSize(),
                'hot_batch_size' => $this->decoyHotBatchSize(),
                ...
                )) + count($this->normalizeIds($router['fresh_suspects'] ?? [])),
                'fresh_waiting' => count($this->normalizeIds(
                    $router['fresh_suspects'] ?? []
                )),
                'dormant_waiting' => count((array) (
    ...
    private function resolveDecoySourcePoolIds(array $router): array
    {
        ...
        foreach (explode(',', (string) ($this->config['decoy_source_pool_ids'] ?? '')) as $ref) {
            ...
            $type = (string) ($router['pools'][$poolId]['type'] ?? '');
            if (
                $poolId !== ''
                && !in_array($type, ['safe', 'danger', 'blacklist'], true)
                && !in_array($poolId, $ids, true)
            ) {
                $ids[] = $poolId;
            }
        }
        $reserved = array_flip(array_filter(array_merge(
            $this->decoySearchPoolIds($router),
            [$this->resolveDecoyCandidatePoolId($router)]
        )));
        $ids = array_values(array_filter(
            $ids,
            fn(string $poolId): bool => !isset($reserved[$poolId])
        ));
        if ($ids === []) {
    ...
    /** 搜索槽必须接收墙 webhook，但不属于普通来源池。 */
    private function decoyMonitoredPoolIds(array $router): array
    {
    ...
    private function decoyIsolateChain(array $router): array
    {
        $chain = [];
        $candidatePoolId = $this->resolveDecoyCandidatePoolId($router);
        $push = function (string $poolId) use (
            &$chain,
            $router,
            $candidatePoolId
        ): void {
            ...
            // 观察1是候选清白池，只能通过双重阴性进入，不能成为自动隔离下游。
            if ($poolId === $candidatePoolId) {
                return;
            }
    ...
    {
        return max(1, (int) ($this->config['decoy_batch_size'] ?? 300));
    }
    ...
    {
        return max(1, (int) (
            $this->config['decoy_verify_batch_size'] ?? 200
        ));
    }
    ...
    {
        return max(300, (int) ($this->config['decoy_observe_minutes'] ?? 180) * 60);
    }

    private function decoyCooldownSeconds(): int
    {
        return max(
            0,
    ...
    {
        if ($this->configBool('decoy_monitor_24h', true)) {
            return true;
        }
    ...
    private function collectPoolPullersInWindow(
        ...
        $lastMap = $ip !== ''
            ? $this->poolIpExposureLastMap($campaign, $poolId, $ip)
            : $this->poolExposureLastMap($campaign, $poolId);
        $ids = [];
    ...
    private function fastIsolateSuspects(
        ...
        foreach ($ids as $uid) {
            ...
            if (
                ...
                || isset($searchPoolMap[$curPool])
                || ($sourcePoolId !== '' && $curPool !== $sourcePoolId)
            ) {
                continue;
            }
    ...
    private function freeSearchSlots(array $router, array $batch = []): array
    {
        ...
        foreach ($this->decoySearchPoolIds($router) as $slot) {
            if (
                (int) (
                    $router['decoy']['domains'][$slot]['cooldown_until'] ?? 0
                ) > time()
            ) {
                continue;
            }
    ...
    private function dispatchSuspectBatch(
        ...
        foreach ($groups as $index => $group) {
            ...
            foreach ($group as $uid) {
                ...
                unset($router['dormant_suspects'][$key]);
                $router['untested_ids'] = array_values(array_diff(
                    $router['untested_ids'] ?? [],
                    [$uid]
                ));
    ...
    private function removeSuspectIdsFromQueues(
        array &$router,
        array $ids,
        bool $removeDormant = false
    ): void {
        $remove = array_flip($this->normalizeIds($ids));
        if ($remove === []) {
            return;
        }
        $router['fresh_suspects'] = array_values(array_filter(
            $this->normalizeIds($router['fresh_suspects'] ?? []),
        ...
        if ($removeDormant) {
            foreach (array_keys($remove) as $uid) {
                unset($router['dormant_suspects'][(string) $uid]);
            }
        }
    }

    /** 最近真实墙事件优先于历史 FIFO，避免活跃嫌疑人在数千人队尾等待。 */
    private function queueFreshSuspects(array &$router, array $ids): int
    {
        $ids = $this->normalizeIds($ids);
        if ($ids === []) {
            return 0;
        }
        $this->removeSuspectIdsFromQueues($router, $ids, true);
        $router['fresh_suspects'] = $this->normalizeIds(array_merge(
            $ids,
            $router['fresh_suspects'] ?? []
        ));
        return count($ids);
    ...
    /** 没有真实拉取记录的用户休眠，下一次拉订阅时再唤醒。 */
    private function hibernateSuspects(array &$router, array $ids, int $now): int
    {
        $ids = $this->normalizeIds($ids);
        if ($ids === []) {
            return 0;
        }
        $this->removeSuspectIdsFromQueues($router, $ids);
        foreach ($ids as $uid) {
            $router['dormant_suspects'][(string) $uid] = $now;
        }
        return count($ids);
    }

    /** 5.3 首次升级：旧大队列全部休眠，按真实拉取逐个唤醒。 */
    private function migrateDecoyQueueStrategy(array &$router, int $now): int
    {
        if ((int) ($router['queue_strategy_version'] ?? 1) >= 2) {
            return 0;
        }
        $freshMap = array_flip($this->normalizeIds($router['fresh_suspects'] ?? []));
        $ids = [];
        foreach ((array) ($router['suspect_queue'] ?? []) as $item) {
        ...
        $ids = array_values(array_filter(
            ...
            fn(int $uid): bool => !isset($freshMap[$uid])
        ));
        $router['suspect_queue'] = [];
        foreach ($ids as $uid) {
            $router['dormant_suspects'][(string) $uid] = $now;
        }
        $router['queue_strategy_version'] = 2;
        return count($ids);
    }

    /**
     * 升级前候选池首墙曾被旧逻辑忽略；利用仍保留的精确 IP 曝光补捞一次。
     *
     * @return list<int>
     */
    private function recoverRecentCandidateWall(
        array &$router,
        array $campaign,
        int $now
    ): array {
        ...
        $candidate = $this->resolveDecoyCandidatePoolId($router);
        if ($candidate === '') {
            return [];
        }
        $event = null;
        foreach (array_reverse((array) ($router['wall_log'] ?? [])) as $item) {
            $at = (int) ($item['at'] ?? 0);
            $eventPool = (string) ($item['pools'][0]['pool_id'] ?? '');
            if (
                $eventPool === $candidate
                && (string) ($item['phase'] ?? '') === 'armed'
                && (int) ($item['moved_count'] ?? 0) === 0
                && $at >= $now - 48 * 3600
            ) {
                $event = $item;
                break;
        ...
        $pullers = $this->collectPoolPullersInWindow(
            ...
            $at - max(
                300,
                (int) ($this->config['wall_fresh_max_seconds'] ?? 7200)
            ),
            $at,
            $oldIp
        );
        $isolate = $this->resolveDecoyIsolatePoolId($router, '');
        $moved = $this->fastIsolateSuspects(
            $router,
            $pullers,
            $now,
            '候选清白池历史首墙补捞→A车道复查',
        ...
        $this->parkProvenIds(
            ...
            $moved,
            $now,
            '候选清白池历史首墙补捞→A车道高风险复查'
        );
        $this->dispatchHotLaneIfFree($router, $now);
        return $moved;
    }

    private function dormantWakeKey(array $campaign): string
    {
        return 'bait_split:dormant_wake:'
            . sha1((string) ($campaign['id'] ?? 'legacy') . ':'
                . (string) ($campaign['generation'] ?? ''));
    }

    /** @return list<int> */
    private function drainDormantWakeQueue(
        array &$router,
        array $campaign,
        int $max = 2000
    ): array {
        try {
            $raw = Redis::zrange($this->dormantWakeKey($campaign), 0, max(0, $max - 1));
        } catch (\Throwable) {
            return [];
        }
        $woken = [];
        $isolate = $this->resolveDecoyIsolatePoolId($router, '');
        foreach ($this->normalizeIds($raw) as $uid) {
            try {
                Redis::zrem($this->dormantWakeKey($campaign), (string) $uid);
            } catch (\Throwable) {
            }
            $key = (string) $uid;
            if (
                !isset($router['dormant_suspects'][$key])
                || $this->routerUserPoolId($router, $uid) !== $isolate
            ) {
                unset($router['dormant_suspects'][$key]);
                continue;
            }
        ...
        if ($woken !== []) {
            $this->queueFreshSuspects($router, $woken);
        }
        return $woken;
    }
    ...
    private function decoyLaneCandidateIds(
        ...
        if ($lane === 'hot') {
            ...
            $ids = $this->normalizeIds($router['fresh_suspects'] ?? []);
            foreach ((array) ($router['suspect_queue'] ?? []) as $item) {
                $ids = array_merge($ids, $this->normalizeIds($item['ids'] ?? []));
            }
            $ids = array_values(array_filter(
        ...
        $ids = array_values(array_filter(
            ...
            function (int $uid) use (&$router, $isolatePool, $active): bool {
                ...
                $key = (string) $uid;
                if (isset($router['dormant_suspects'][$key])) {
                    return false;
                }
        ...
        // 普通初筛严格按入队顺序，来源池反复被墙不能把高活跃正常用户
        // 反复顶到队首；只有已有阳性证据的车道才按风险排序。
        if ($lane === 'bulk') {
    ...
    /**
     * @return list<array{lane:string,dispatched:int,slots:array}>
     */
    private function dispatchSearchLanes(array &$router, int $now): array
    {
        $searchSlots = $this->decoySearchPoolIds($router);
        $freeSlots = $this->freeSearchSlots($router);
        $dispatched = [];
        $dispatchLane = function (string $lane, string $slot) use (
            &$router,
            &$freeSlots,
            &$dispatched,
            $now
        ): bool {
            if ($slot === '' || !in_array($slot, $freeSlots, true)) {
                return false;
            }
    ...
    private function dispatchHotLaneIfFree(array &$router, int $now): int
    {
        $hotSlot = (string) ($this->decoySearchPoolIds($router)[0] ?? '');
        if (
            $hotSlot === ''
            || !in_array($hotSlot, $this->freeSearchSlots($router), true)
        ) {
            return 0;
        }
        $result = $this->dispatchDecoyLane($router, $hotSlot, 'hot', $now);
        return (int) ($result['dispatched'] ?? 0);
    }

    /**
     * 已取证的组暂时收进重点嫌疑名单，扣在观察3 里等空槽复测。
     * 用在两处：天亮收工、以及这一轮组里一个人都没拉订阅（等于什么都没测到，
     * 不能当清白放回默认组，一夜换来的线索就这么断了）。
     */
    private function parkProvenGroup(
        array &$router,
        string $slot,
        int $now,
        string $note
    ): int {
        return $this->parkProvenIds(
            $router,
    ...
    /**
     * A 槽独占高风险收敛。升级前遗留在 B/C 的高风险分支先保留证据，
     * 收回观察3等待 A 槽，避免三条车道再次全部被阳性分支占满。
     */
    private function rebalanceSearchLaneRoles(array &$router, int $now): int
    {
        $slots = $this->decoySearchPoolIds($router);
        $hotSlot = (string) ($slots[0] ?? '');
        if ($hotSlot === '') {
            return 0;
        }
        $parked = 0;
        foreach (array_slice($slots, 1) as $slot) {
            $domain = $this->normalizeDecoyDomain(
                $router['decoy']['domains'][$slot] ?? []
            );
            if ((string) ($domain['lane'] ?? '') !== 'hot') {
                continue;
            }
            ...
            if ($members !== []) {
                $domain['last_group'] = [
                    'ids' => $members,
                    'ip' => (string) ($router['pools'][$slot]['host'] ?? ''),
                    'from' => (int) ($domain['dispatched_at'] ?? 0),
                    'to' => $now,
                ];
                $parked += $this->parkProvenIds(
                    $router,
                    $members,
                    $now,
                    '高风险分支转交搜索槽A继续收敛'
            ...
            $router['decoy']['domains'][$slot] = array_merge($domain, [
                'phase' => 'idle',
                'armed' => false,
                'armed_at' => 0,
                'dispatched_at' => 0,
        ...
        if ($parked > 0) {
            Log::notice('BaitSplit 高风险分支归并到A车道候测', [
                'parked' => $parked,
                'hot_slot' => $hotSlot,
            ]);
        }
    ...
    /** 同上，但直接指定人。 */
    private function parkProvenIds(
        array &$router,
        array $ids,
        int $now,
        string $note
    ): int {
        $keepPoolId = $this->resolveDecoyIsolatePoolId($router, '');
        $ids = $this->normalizeIds($ids);
        $this->removeSuspectIdsFromQueues($router, $ids, true);
        ...
        foreach ($ids as $uid) {
            ...
            if ($keepPoolId !== '') {
                ...
                $router['assignments'][$key] = $keepPoolId;
            }
            unset(
                $router['search_origin'][$key],
                $router['candidate_users'][$key],
                $router['clean_rounds'][$key]
            );
            $carry[] = (int) $uid;
            $parked++;
        }
        $router['decoy']['carryover'] = $this->normalizeIds($carry);
        return $parked;
    }

    /**
     * 墙迟到了：本组没人拉过这个 IP，但上一组人在这个 IP 上拉过。
     * 把上一组里拉过的人捞回来当嫌疑，别让证据烂在放行记录里。
     */
    private function recallLateWallGroup(
        array $campaign,
        array &$router,
        string $slot,
        string $deadIp,
        ...
        $last = (array) ($router['decoy']['domains'][$slot]['last_group'] ?? []);
        $ids = $this->normalizeIds($last['ids'] ?? []);
        $to = (int) ($last['to'] ?? 0);
        $from = (int) ($last['from'] ?? 0);
        if ($ids === [] || $to <= 0) {
            return [];
        }
        // 只认最近一小时内放走的，再久就说不清是谁了
        if ($now - $to > 3600) {
            return [];
        }
        // IP 得对得上：放走时槽用的还是这个刚死的 IP
        if ($deadIp !== '' && (string) ($last['ip'] ?? '') !== $deadIp) {
            return [];
        }
        $pulled = array_values(array_intersect(
            $ids,
        ...
        Log::warning('BaitSplit 迟到的墙：追回上一组已放行的人', [
            'pool_id' => $slot,
            'dead_ip' => $deadIp,
            'released_ago_minutes' => (int) round(($now - $to) / 60),
            'recalled' => count($pulled),
        ]);
        return $pulled;
    }
    ...
    private function releaseSearchSlotUser(
        ...
        $home = $isolate;
        if ($graduate) {
            $rounds = (int) ($router['clean_rounds'][$key] ?? 0) + 1;
            $router['clean_rounds'][$key] = $rounds;
            if ($rounds >= 2) {
                $candidate = $this->resolveDecoyCandidatePoolId($router);
                if ($candidate !== '') {
                    $home = $candidate;
                    $note = '候选清白观察（已完成 ' . $rounds . ' 次独立阴性测试）';
                    $router['candidate_users'][$key] = [
                        'entered_at' => $now,
                        // 首次 cron 时记录当时累计拉取数作为基线
                        'baseline_pulls' => -1,
                        'ready_at' => 0,
                    ];
                }
            } else {
                $note = '分组测试阴性第 ' . $rounds . ' 轮（留观察3待第二轮）';
            }
        } else {
            $note = $note !== '' ? $note : '分组测试无结论（回观察3待复测）';
        }
        if ($home === '') {
            return 0;
        }
        ...
        if ($home !== $this->resolveDecoyCandidatePoolId($router)) {
    ...
    private function releaseSearchSlot(
        ...
        string $note,
        bool $graduate = true,
        ?array $graduateIds = null
    ): int {
        // 记下这一组看过哪个 IP、看的时间段：上报到封锁有延迟，
        // 墙常常在人放走之后才到，得能倒查是谁干的
        $members = $this->poolLockedMemberIds($router, $slot);
        if ($members !== [] && isset($router['decoy']['domains'][$slot])) {
            $router['decoy']['domains'][$slot]['last_group'] = [
                'ids' => $members,
                'ip' => (string) ($router['pools'][$slot]['host'] ?? ''),
                'from' => (int) (
        ...
        $graduateMap = $graduateIds === null
            ? null
            : array_flip($this->normalizeIds($graduateIds));
        $released = 0;
        foreach ($this->poolLockedMemberIds($router, $slot) as $uid) {
            $released += $this->releaseSearchSlotUser(
                ...
                false,
                $graduate && (
                    $graduateMap === null || isset($graduateMap[$uid])
                )
            );
        }
        if (isset($router['decoy']['domains'][$slot])) {
    ...
    private function convictSuspects(array &$router, array $ids, int $now): array
    {
        ...
        if ($target === '') {
            Log::error('BaitSplit 定罪失败：未配置危险组', ['ids' => $ids]);
            $this->parkProvenIds(
                $router,
                $ids,
                $now,
                '分组测试·已双重确认但危险组缺失（保留观察3）'
            );
            return [];
        }
        $this->removeSuspectIdsFromQueues($router, $ids, true);
    ...
    /**
     * 观察1只做候选清白池：满 72 小时且至少拉取 3 次后仅标记待人工确认，
     * 永远不自动进入安全组。
     */
    /**
    ...
    private function refreshCandidateUsers(
        ...
        foreach ($ids as $uid) {
            ...
            $count = (int) ($stats[$uid]['count'] ?? 0);
            if ((int) ($item['baseline_pulls'] ?? -1) < 0) {
                $router['candidate_users'][$key]['baseline_pulls'] = $count;
                continue;
            }
            $elapsed = $now - (int) ($item['entered_at'] ?? 0);
            $pulls = $count - (int) ($item['baseline_pulls'] ?? 0);
            if (
                $elapsed < 72 * 3600
                || $pulls < 3
                || (int) ($item['ready_at'] ?? 0) > 0
            ) {
                continue;
            }
            $router['candidate_users'][$key]['ready_at'] = $now;
            $router['overrides'][$key] = $this->normalizeOverride([
                'pool_id' => $poolId,
                'locked' => true,
                'note' => '候选清白·待人工确认（72小时无墙，实际拉取 '
                    . $pulls . ' 次）',
                'updated_at' => $now,
            ]);
    ...
    private function decoyRolloverIfNeeded(
        ...
        if ($this->configBool('decoy_monitor_24h', true)) {
            $router['decoy']['active_date'] = $today;
            return true;
        }
        ...
        $router['decoy'] = [
            'active_date' => $today,
            'domains' => [],
            'carryover' => $this->normalizeIds($router['decoy']['carryover'] ?? []),
        ];
        return true;
    }
    ...
    private function decoyRecallAll(array &$router, int $now): int
    {
        ...
        // 但已取证的链拆到最后剩的这几个人是一整夜换来的最强线索，
        // 当清白放回默认组就白干了——扣在观察3 里，今晚开工先测他们
        $parked = 0;
        foreach ($this->decoySearchPoolIds($router) as $slot) {
            if (!empty($router['decoy']['domains'][$slot]['proven'])) {
                $parked += $this->parkProvenGroup(
                    $router,
                    $slot,
                    $now,
                    '分组测试·重点嫌疑（昨夜收敛未定罪，今晚续测）'
                );
            }
        ...
        if ($parked > 0) {
            Log::warning('BaitSplit 天亮保留重点嫌疑', [
                'ids' => $router['decoy']['carryover'] ?? [],
            ]);
        }
        $router['suspect_queue'] = [];
        ...
        foreach ($router['overrides'] as $key => $ov) {
            ...
            if (
                ...
                || str_contains($note, '金丝雀观察正常')
                // 重点嫌疑要扣在观察3 里过白天，今晚续测，别跟着一起清掉
                || str_contains($note, '重点嫌疑')
                || str_contains($note, '候选清白')
            ) {
                continue;
            }
            $isDecoy = false;
    ...
    private function handleDecoyMachineIp(
        ...
        // 机器换 IP 后旧会话立即失效，必须从新 IP 重新记录精确曝光。
        if ($phase === 'testing') {
        ...
        if ($phase !== 'idle') {
            ...
            $members = $this->poolLockedMemberIds($router, $poolId);
            if (
                in_array($poolId, $this->decoySearchPoolIds($router), true)
                && $members !== []
            ) {
                $router['decoy']['domains'][$poolId]['dispatched_at'] = $now;
                $router['decoy']['domains'][$poolId]['group_size'] = count($members);
            }
        }
        if ($poolId === $this->resolveDecoyCandidatePoolId($router)) {
            foreach ($router['candidate_users'] as &$candidate) {
                $candidate['entered_at'] = $now;
                $candidate['baseline_pulls'] = -1;
    ...
    private function handleDecoyWall(
        ...
        if ($phase === 'idle' && $poolId === $candidatePoolId) {
            // 候选池里的用户已经完成两轮阴性测试；该池首次墙也不能再忽略。
            // 只追回最近真实拿过旧 IP 的人，直接进入 A 高风险复查。
            ...
            $isolateTargetId = $this->resolveDecoyIsolatePoolId($router, '');
            ...
            $hotDispatched = $this->dispatchHotLaneIfFree($router, $now);
            foreach ($router['candidate_users'] as &$candidate) {
                $candidate['entered_at'] = $now;
                $candidate['baseline_pulls'] = -1;
                $candidate['ready_at'] = 0;
            ...
            $phase = 'armed';
            $logPhase = 'candidate_recheck_first';
            Log::warning('BaitSplit 候选清白池首次墙立即复查', [
                'pool_id' => $poolId,
                'pullers' => count($pullers),
                'hot_queued' => count($movedIds),
                'hot_dispatched' => $hotDispatched,
            ]);
        } elseif ($phase === 'idle') {
            // 第一次墙：不管人，只武装新 IP
            $this->applyPoolHost($router, $poolId, $newIp);
            ...
            $suspectSeeded = count($pullers);
            if ($candidatePoolId !== '' && $poolId === $candidatePoolId) {
                // 候选清白池再墙：只退回确实拉过该精确 IP 的人；其余人
                // 继续可用，但 72 小时观察重新计时。
                $isolateTargetId = $this->resolveDecoyIsolatePoolId($router, '');
                $movedIds = $this->fastIsolateSuspects(
                    $router,
                ...
                Log::warning('BaitSplit 候选清白池再墙', [
                    'pool_id' => $poolId,
                    'pullers' => count($pullers),
                    'hot_queued' => $isolated,
                    'hot_dispatched' => $hotDispatched,
                ]);
            } elseif (in_array($poolId, $searchSlots, true)) {
                // 搜索槽被墙：内鬼就在本组拉过订阅的人里，同组没拉的当场放回去
                $group = $this->poolLockedMemberIds($router, $poolId);
                $hit = array_values(array_intersect($group, $pullers));
                $noInfo = array_values(array_diff($group, $hit));
                // 上一组人可能已经过了观察期被放走，墙才姗姗来迟。
                // 实测 21% 的墙是这样迟到的，不倒查就会算到下一批无辜的人头上
                $recalled = [];
                $lateRecall = false;
                if ($hit === []) {
                    $recalled = $this->recallLateWallGroup(
                        $campaign,
                        $router,
                        $poolId,
                        $armedIp,
                    ...
                    if ($recalled !== []) {
                        $hit = $recalled;
                        // 迟到的墙属于上一组；当前组不能继续沿用已经换掉的 IP。
                        // 将当前组按无结论回队，避免观察计时和 IP 会话错位。
                        $noInfo = $group;
                        $lateRecall = true;
                    }
                }
                $cleared = 0;
                foreach ($noInfo as $uid) {
                    $cleared += $this->releaseSearchSlotUser(
                        $router,
                        (int) $uid,
                        $now,
                        '分组测试无结论（本轮未拉订阅，回观察3待复测）',
                        false,
                        false
                    );
                }
                if ($noInfo !== []) {
                    $this->hibernateSuspects($router, $noInfo, $now);
                }
                $weight = 1 / max(1, count($hit));
                foreach ($hit as $uid) {
                    $key = (string) $uid;
                    $router['wall_hits'][$key] =
                        (int) ($router['wall_hits'][$key] ?? 0) + 1;
                ...
                if ($hit === []) {
                    ...
                    foreach ($hit as $uid) {
                        $key = (string) $uid;
                        $rounds = (int) ($router['positive_rounds'][$key] ?? 0) + 1;
                        $router['positive_rounds'][$key] = $rounds;
                        if ($rounds >= 2) {
                            $ready[] = $uid;
                        }
                    }
                    if ($ready !== []) {
                        $movedIds = $this->convictSuspects($router, $ready, $now);
                        $isolated = count($movedIds);
                        $isolateTargetId = $this->resolveDecoyConfirmPoolId($router);
                        $logPhase = 'convicted';
                    } else {
                        $isolated = $this->parkProvenIds(
                            $router,
                            $hit,
                            $now,
                            '分组测试·单人首次阳性（待独立新 IP 二次确认）'
                        );
                        $movedIds = $hit;
                        $logPhase = 'high_risk';
                        if ($poolId !== $hotSlot) {
                    ...
                    $router['decoy']['domains'][$poolId]['dispatched_at'] = 0;
                    $router['decoy']['domains'][$poolId]['group_size'] = 0;
                    Log::warning('BaitSplit 分组测试单人命中', [
                        'pool_id' => $poolId,
                        'ids' => $movedIds,
                        'confirmed' => $logPhase === 'convicted',
                        'released_same_group' => $cleared,
                    ]);
                } else {
                    $note = '分组测试·重点嫌疑（细分剔下的另一半，等空槽复测）';
                    if ($poolId !== $hotSlot) {
                        // B/C 的阳性证据只入高风险队列，由 A 槽继续拆分。
                        // 绝不让初筛、二验槽被高风险分支长期占用。
                        $isolated = $this->parkProvenIds(
                            $router,
                            $hit,
                        ...
                        $movedIds = $hit;
                        $logPhase = 'deferred_hot';
                        $hotDispatched = $this->dispatchHotLaneIfFree($router, $now);
                        Log::notice('BaitSplit B/C命中转交A车道', [
                            'pool_id' => $poolId,
                            'hit' => count($hit),
                            'released_same_group' => $cleared,
                            'hot_dispatched' => $hotDispatched,
                        ...
                        // A 槽只在自己槽内继续拆，不能借用 B/C。
                        $deferred = [];
                        if (count($hit) > 1) {
                            $half = (int) ceil(count($hit) / 2);
                            $deferred = array_slice($hit, $half);
                            $hit = array_slice($hit, 0, $half);
                            $this->parkProvenIds($router, $deferred, $now, $note);
                        }
                        $split = $this->dispatchSuspectBatch(
                            $router,
                            $hit,
                            '',
                            $now,
                        ...
                        $logPhase = 'split';
                        Log::notice('BaitSplit A车道高风险细分', [
                            'pool_id' => $poolId,
                            'hit' => count($hit),
                            'deferred' => count($deferred),
                            'released_same_group' => $cleared,
                            'late_recall' => $lateRecall,
                        ]);
                    }
                }
                $this->applyPoolHost($router, $poolId, $newIp);
                ...
                if (
                    $poolId !== $hotSlot
                    && $this->poolLockedMemberIds($router, $poolId) === []
                ) {
                    $router['decoy']['domains'][$poolId] = array_merge(
                        $this->normalizeDecoyDomain(
                            $router['decoy']['domains'][$poolId]
                ...
                // 来源池止血优先：见过死 IP 的人先全部移出来源池到观察3，
                // 再从中取一小批进搜索槽。未排上槽的人也不能留在默认组继续拿新 IP。
                $holdPoolId = $this->resolveDecoyIsolatePoolId($router, '');
                $parkedIds = $this->fastIsolateSuspects(
                    $router,
                    $pullers,
                    $now,
                    '精确 IP 二墙拉取者→观察3候测',
                ...
                if ($parkedIds !== []) {
                    $this->queueFreshSuspects($router, $parkedIds);
                }
                $movedIds = $parkedIds;
    ...
    private function runDecoyLocked(): array
    {
        ...
        foreach ($state['campaigns'] as $id => $campaign) {
            ...
            if (!$inWindow) {
                if (
                    ...
                    $router['decoy'] = [
                        'active_date' => '',
                        'domains' => [],
                        'carryover' => $this->normalizeIds(
                            $router['decoy']['carryover'] ?? []
                        ),
                    ];
                    $router['config_version']++;
                    $changed = true;
            ...
            $recoveredCandidateIds = $this->recoverRecentCandidateWall(
                $router,
                $campaign,
                $now
            );
            if ($recoveredCandidateIds !== []) {
                $touched = true;
                $actions[] = [
                    'campaign_id' => $id,
                    'action' => 'candidate_wall_recovered',
                    'users' => count($recoveredCandidateIds),
                ];
                Log::warning('BaitSplit 补捞升级前候选池首墙拉取者', [
                    'users' => count($recoveredCandidateIds),
                ]);
            }
            $migrated = $this->migrateDecoyQueueStrategy($router, $now);
            if ($migrated > 0) {
                $touched = true;
                $actions[] = [
                    'campaign_id' => $id,
                    'action' => 'queue_hibernated',
                    'users' => $migrated,
                ];
                Log::notice('BaitSplit 历史初筛队列转为按拉取唤醒', [
                    'users' => $migrated,
                ]);
            }
            $woken = $this->drainDormantWakeQueue($router, $campaign);
            if ($woken !== []) {
                $touched = true;
                $actions[] = [
                    'campaign_id' => $id,
            ...
            $rebalanced = $this->rebalanceSearchLaneRoles($router, $now);
            if ($rebalanced > 0) {
                $touched = true;
                $actions[] = [
                    'campaign_id' => $id,
                    'action' => 'lane_rebalance',
                    'parked' => $rebalanced,
                ];
            }
            ...
            foreach ($this->decoySearchPoolIds($router) as $slot) {
                ...
                $slotIp = (string) (
                    $router['decoy']['domains'][$slot]['armed_ip']
                    ?? $router['pools'][$slot]['host']
                    ?? ''
                );
                $members = $this->poolLockedMemberIds($router, $slot);
                ...
                if ($noInfoIds !== []) {
                    $this->hibernateSuspects($router, $noInfoIds, $now);
                }
                if ($graduatedIds !== []) {
                    $last = (array) (
                        $router['decoy']['domains'][$slot]['last_group'] ?? []
            ...
            $candidateResult = $this->refreshCandidateUsers(
                $campaign,
                $router,
                $now
            );
            if ($candidateResult['adopted'] > 0) {
                $touched = true;
                $actions[] = [
                    'campaign_id' => $id,
                    'action' => 'candidate_adopted',
                    'users' => $candidateResult['adopted'],
                ];
                Log::notice('BaitSplit 候选组旧成员纳入72小时跟踪', [
                    'users' => $candidateResult['adopted'],
                ]);
            }
            if ($candidateResult['ready'] > 0) {
                $touched = true;
                $actions[] = [
                    'campaign_id' => $id,
