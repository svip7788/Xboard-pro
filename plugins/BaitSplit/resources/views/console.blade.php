...
<main class="page">
    ...
    <div class="grid">
        ...
        <section class="card wide">
            ...
            <div class="split">
                <div class="field"><label>确认内鬼池（追猎定罪后转入）</label><select id="decoyConfirm"></select></div>
            </div>

            <button id="saveWallSettings">保存跟墙设置</button>
...
fillSelect('decoyConfirm',pools(),settings.confirm_pool_raw||'','自动（危险组）');renderHunt()}
...
$('saveWallSettings').onclick=async()=>{try{const body={wall_auto_isolate:$('wallAutoIsolate').checked,wall_hit_threshold:Number($('wallThreshold').value)||2,wall_lookback_seconds:Number($('wallLookback').value)||3600,wall_fresh_max_seconds:Number($('wallFresh').value)||7200,wall_observe_pool_id:$('wallObservePool').value,decoy_confirm_pool_id:$('decoyConfirm').value};await request('/wall-settings',{method:'POST',body:JSON.stringify(body)});toast('跟墙设置已保存');await loadWallLog()}catch(error){toast(error.message,'error')}};
