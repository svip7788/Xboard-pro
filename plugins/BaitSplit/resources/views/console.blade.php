...
function openTreeTransfer(node){const targets=pools(['default','probe','observation','safe','custom','emergency']).filter(item=>item.status!=='blocked');if(!targets.length)return toast('暂无可用的普通用户池','error');transferSource={...node,mode:'tree'};fillSelect('transferTarget',targets,'');$('transferHint').textContent=`“${node.name}”共有 ${node.user_count} 名固定用户；可迁移到默认、测试、观察、安全等普通用户池，手动锁定用户不会移动。`;$('transferModal').classList.add('show')}
...
function renderInvestigationTree(){
    ...
    ordered.forEach(node=>{
        ...
        if(node.status!=='archived'&&!node.children.length){
            ...
            if(node.status==='safe')add('一键迁移到用户池',()=>openTreeTransfer(node),'success');
        }
