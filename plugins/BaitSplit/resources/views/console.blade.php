...
<main class="page">
    ...
    <div class="grid">
        ...
        <section class="card wide">
            ...
            <h3 style="margin-top:20px">二分收敛诱捕（零额外 IP 揪内鬼）<span id="decoyState" class="pill off" style="margin-left:8px">未启用</span></h3>
            <div class="hint">墙时段来源池被墙换 IP 时，新 IP 只发一小批（其余人留死 IP，深夜没人用）；这批又被墙 → 内鬼在这批：批&gt;收敛下限则对半拆、移入隔离中转池续测；批≤收敛下限仍被墙则整批判定内鬼进危险组。没被墙 = 清白，收回换下一批。多个来源池各自独立并行，到点全组解析恢复。</div>
            <div class="split" style="margin-top:12px">
            ...
            <div class="field"><label>嫌疑来源池（内鬼现在藏在哪些池，可多选，各自独立并行）</label><div id="decoySource" class="group-list"></div></div>
...
const g=$('decoyGroups');g.textContent='';(d.domains||[]).forEach(dm=>{const card=document.createElement('div');card.className='pool';const head=document.createElement('div');head.className='pool-head';const t=document.createElement('strong');t.textContent=dm.pool_name;const s=document.createElement('span');const testing=(dm.current_batch||0)>0;s.className=`pill ${testing?'on':'off'}`;s.textContent=testing?`在测 ${dm.current_batch} 人`:(dm.cur_ip?'待抽批':'未触发');head.append(t,s);const meta=document.createElement('div');meta.className='meta';meta.textContent=`当前 IP ${dm.cur_ip||'无（等被墙）'} · 本批粒度 ${dm.current_level||'-'} 人 · 待细分 ${dm.queue_users||0} 人（${dm.queue_items||0} 组）`;card.append(head,meta);g.appendChild(card)});if(!(d.domains||[]).length){g.innerHTML='<div class="empty">暂无来源池（请勾选嫌疑来源池并启用）</div>'}}
