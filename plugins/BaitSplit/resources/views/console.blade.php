...
<main class="page">
    ...
    <div class="grid">
        ...
        <section class="card wide">
            ...
            <div class="hint">勾选一个或多个最上层根组：已拉取用户进入新树，未拉取用户回到默认容量链；成功后旧树和旧分组会直接删除。</div>
...
function blankCampaign(){const first=meta.groups[0]?.id||0;return{id:'',name:'',target_group_id:first,target_group_ids:first?[first]:[],target_server_ids:[],excluded_server_ids:[],eligible_count:0,router:null}}
...
function renderGroups(){const container=$('groupSelect');container.textContent='';const selected=new Set((current?.target_group_ids||[current?.target_group_id]).map(Number));meta.groups.forEach(group=>{const label=document.createElement('label');label.className='group-option';const input=document.createElement('input');input.type='checkbox';input.className='targetGroup';input.value=group.id;input.checked=selected.has(Number(group.id));input.onchange=renderNodes;const text=document.createElement('span');text.textContent=`${group.name}（${group.users_count} 人）`;label.append(input,text);container.appendChild(label)})}
...
function openTreeTransfer(node){const targets=pools(['default','probe','observation','safe','custom','emergency']).filter(item=>item.id!==node.pool_id&&item.status!=='blocked');if(!targets.length)return toast('暂无可用的普通用户池','error');transferSource={...node,mode:'tree'};fillSelect('transferTarget',targets,'');$('transferHint').textContent=`“${node.name}”已有 ${node.pulled_count} 人拉取；只迁移这些已拉取用户，剩余 ${Math.max(0,node.user_count-node.pulled_count)} 人继续留在当前分支观察。手动锁定用户不会移动。`;$('transferModal').classList.add('show')}
async function poolAction(action,pool){try{if(action==='edit')return editPool(pool);if(action==='users')return showPoolUsers(pool);if(action==='transfer')return openPoolTransfer(pool);if(action==='tree'){if(!confirm(`把“${pool.name}”的 ${pool.member_count} 名固定用户冻结为独立排查根组？\n创建后还需继续拆分并填写新域名。`))return;updateCurrent(await request(api(`/pools/${encodeURIComponent(pool.id)}/investigation`),{method:'POST',body:JSON.stringify({name:''})}));toast('已创建排查根组，请继续拆分');document.getElementById('investigationTree').scrollIntoView({behavior:'smooth'});return}if(action==='delete'){if(!confirm(`删除“${pool.name}”？`))return;updateCurrent(await request(api(`/pools/${encodeURIComponent(pool.id)}`),{method:'DELETE'}));toast('用户池已删除')}}catch(error){toast(error.message,'error')}}
...
function openMergeTree(){const nodes=router()?.investigation_nodes||[],selected=nodes.filter(node=>mergeTreeNodeIds.has(node.id));if(!selected.length)return toast('请至少选择一个最上层根组','error');$('mergeTreeHint').textContent=`已选择 ${selected.length} 棵旧树，共 ${selected.reduce((sum,node)=>sum+treeMergeableCount(node.id,nodes),0)} 名已拉取用户。未拉取用户回到默认容量链，成功后旧树直接删除。`;$('mergeTreeName').value=selected.length>1?'合并排查树':'重组排查树';$('mergeTreeCount').value=2;renderMergeTreeFields();$('mergeTreeModal').classList.add('show')}
...
$('saveCampaign').onclick=async()=>{try{const result=await request('/campaigns',{method:'POST',body:JSON.stringify({campaign_id:current.id||null,name:$('campaignName').value.trim(),target_group_ids:selectedGroupIds(),excluded_server_ids:excludedServerIds()})});updateCurrent(result);toast('任务已保存')}catch(error){toast(error.message,'error')}};
...
$('closeTransfer').onclick=()=>{$('transferModal').classList.remove('show');transferSource=null};$('transferModal').onclick=event=>{if(event.target===$('transferModal'))$('closeTransfer').click()};$('confirmTransfer').onclick=async()=>{if(!transferSource)return;const source=transferSource,targetId=$('transferTarget').value,targetName=$('transferTarget').options[$('transferTarget').selectedIndex]?.textContent,isTree=source.mode==='tree';if(!confirm(`把“${source.name}”已拉取用户转入“${targetName}”？${isTree?'\n未拉取用户继续留在当前分支观察。':''}`))return;try{const path=isTree?`/investigations/${encodeURIComponent(source.id)}/move`:`/pools/${encodeURIComponent(source.id)}/move-pulled`;const result=await request(api(path),{method:'POST',body:JSON.stringify({target_pool_id:targetId})});updateCurrent(result.campaign);$('closeTransfer').click();toast(isTree?`已移动 ${result.moved_count} 名已拉取用户，原分支剩余 ${result.remaining_count} 人`:`已移动 ${result.moved_count} 人`)}catch(error){toast(error.message,'error')}};
$('closeTreeHost').onclick=()=> $('treeHostModal').classList.remove('show');$('treeHostModal').onclick=event=>{if(event.target===$('treeHostModal'))$('closeTreeHost').click()};$('treeHostValue').onkeydown=event=>{if(event.key==='Enter')$('confirmTreeHost').click()};$('confirmTreeHost').onclick=async()=>{const nodeId=$('treeHostNodeId').value,host=$('treeHostValue').value.trim();if(!nodeId||!host)return toast('请输入域名或 IP','error');try{updateCurrent(await request(api(`/investigations/${encodeURIComponent(nodeId)}/host`),{method:'POST',body:JSON.stringify({host})}));$('closeTreeHost').click();toast('分支域名/IP已更新，下次拉取立即生效')}catch(error){toast(error.message,'error')}};
