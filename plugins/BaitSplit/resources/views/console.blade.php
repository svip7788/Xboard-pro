...
<main class="page">
    ...
    <div class="grid">
        ...
        <section class="card wide">
            ...
            <div class="hint">墙压倒性落在凌晨、白天近乎为零，形态是按地址批量探测，而探测只找得到有流量的地址。开启后窗口内把普通用户挤到牺牲池，拿一两个地址换其余几个的存活。人工指定了用户池的用户不参与。牺牲池选两个以上时，同一个人的节点会分散下发到多个地址，其中一个被墙时另一半节点还能连。</div>
            ...
            <div class="field"><label>牺牲池（可多选，建议选两个）</label><div id="convergePools" class="branch-fields"></div></div>
            <div class="split">
...
list.forEach(pool=>{const label=document.createElement('label');label.style.cssText='display:flex;align-items:center;gap:6px;font-weight:400';const box2=document.createElement('input');box2.type='checkbox';box2.value=pool.id;box2.dataset.role='convergePool';box2.style.cssText='width:auto;height:auto';box2.checked=selected.has(pool.id);label.appendChild(box2);label.appendChild(document.createTextNode(`${pool.name}（${pool.host||'无地址'}）`));box.appendChild(label)})}
