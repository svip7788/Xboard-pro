...
const g=$('decoyGroups');g.textContent='';const slotIds=new Set((d.search_pool_ids||[]).map(String));(d.domains||[]).forEach(dm=>{const card=document.createElement('div');card.className='pool';const head=document.createElement('div');head.className='pool-head';const t=document.createElement('strong');t.textContent=dm.pool_name;const s=document.createElement('span');const phase=dm.phase||'idle';const testing=(dm.current_batch||0)>0;const isSlot=slotIds.has(String(dm.pool_id));const grp=Number(dm.group_size||0);const cooling=Number(dm.cooldown_until||0)>Math.floor(Date.now()/1000);let label='未触发',cls='off';if(isSlot){if(grp>0){label=dm.proven?`搜索槽·阳性分支 ${grp} 人`:`搜索槽·在测 ${grp} 人`;cls=dm.proven?'bad':'on'}else if(cooling){label='搜索槽·冷却中';cls='warn'}else{label='搜索槽·空闲';cls='off'}}else if(phase==='armed'){label='已武装·精确IP监控';cls='warn'}else if(phase==='testing'){label='旧测试态·将清理';cls='off'}else if(testing){label=`残留在测 ${dm.current_batch}`;cls='off'}s.className=`pill ${cls}`;s.textContent=label;head.append(t,s);const meta=document.createElement('div');meta.className='meta';meta.textContent=isSlot?`阶段 ${phase} · 当前IP ${dm.cur_ip||'-'}${grp>0?` · 本组 ${grp} 人 · 每人首次拉取后独立观察 ${d.observe_minutes||180} 分钟`:cooling?' · 防迟到墙冷却中':' · 等待固定批次'}`:`阶段 ${phase} · 武装IP ${dm.armed_ip||'-'} · 当前IP ${dm.cur_ip||'-'}（再墙只处理实际拉过该精确IP的人）`;card.append(head,meta);g.appendChild(card)});if(!(d.domains||[]).length){g.innerHTML='<div class="empty">暂无监控池（等待首次墙事件武装）</div>'}}
