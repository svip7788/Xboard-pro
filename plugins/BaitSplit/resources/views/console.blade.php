...
<main class="page">
    ...
    <div class="grid">
        ...
        <section class="card wide">
            <div class="topbar">
                <h2>跟墙检测（被墙自动隔离）</h2>
                <div class="actions"><span id="wallTracked" class="pill off">累计跟踪 0 人</span><button id="refreshWall" class="secondary">刷新</button></div>
            </div>
            ...
            <h3 style="margin-top:20px">金丝雀分批诱捕（省 IP 揪内鬼）<span id="decoyState" class="pill off" style="margin-left:8px">未启用</span></h3>
            <div class="hint">墙时段每隔「观察时长」从来源池抽一小批（优先已拉取者）指向金丝雀池的新 IP，其余人留旧 IP（旧 IP 死了也无妨，深夜没人用）；这批 IP 被墙 → 内鬼就在这小批，全体记跟墙分，累计达阈值自动进危险组；没被墙就收回换下一批。只要 1 个金丝雀独立 IP 即可。</div>
            <div class="split" style="margin-top:12px">
            ...
            <div class="split">
                <div class="field"><label>每批人数</label><input id="decoyBatch" type="number" min="1" max="5000" value="80"></div>
            ...
            <div class="field"><label>② 金丝雀池（每批抽出的小批人指到这里的新 IP 观察；被墙即锁定嫌疑到该批。需 ≥1 个各自独立干净 IP、且不能是来源池）</label><div id="decoyPools" class="group-list"></div></div>

            <button id="saveWallSettings">保存跟墙 / 诱捕设置</button>

            <h3 style="margin-top:20px">当前批次</h3>
...
if(document.activeElement!==$('decoyEnd'))$('decoyEnd').value=d.end||'09:00';
if(document.activeElement!==$('decoyBatch'))$('decoyBatch').value=d.batch_size||80;
...
const srcBox=$('decoySource');srcBox.textContent='';const srcSel=new Set((d.source_pool_ids||[]).map(String));all.forEach(p=>{const label=document.createElement('label');label.className='group-option';const input=document.createElement('input');input.type='checkbox';input.className='decoySource';input.value=p.id;input.checked=srcSel.has(String(p.id));const span=document.createElement('span');span.textContent=`${p.name}（${p.host||'无IP'}）`;label.append(input,span);srcBox.appendChild(label)});
const box=$('decoyPools');box.textContent='';const selected=new Set((d.decoy_pool_ids||[]).map(String));all.filter(p=>!['danger','blacklist'].includes(p.type)).forEach(p=>{const label=document.createElement('label');label.className='group-option';const input=document.createElement('input');input.type='checkbox';input.className='decoyPool';input.value=p.id;input.checked=selected.has(String(p.id));const span=document.createElement('span');span.textContent=`${p.name}（${p.host||'无IP'}）`;label.append(input,span);box.appendChild(label)});
const g=$('decoyGroups');g.textContent='';(d.groups||[]).forEach(gr=>{const card=document.createElement('div');card.className='pool';const head=document.createElement('div');head.className='pool-head';const t=document.createElement('strong');t.textContent=gr.pool_name;const s=document.createElement('span');s.className=`pill ${gr.walled?'bad':'on'}`;s.textContent=gr.walled?'已被墙·嫌疑锁定':'观察中';head.append(t,s);const meta=document.createElement('div');meta.className='meta';meta.textContent=`${gr.host||'无IP'} · 本批 ${gr.count} 人 · 今晚已测 ${d.tested_count||0} 人`;card.append(head,meta);g.appendChild(card)});if(!(d.groups||[]).length){g.innerHTML='<div class="empty">当前无批次（到诱捕时段自动抽批）</div>'}}
...
$('saveWallSettings').onclick=async()=>{try{const decoyPools=[...document.querySelectorAll('.decoyPool:checked')].map(i=>i.value);const decoySources=[...document.querySelectorAll('.decoySource:checked')].map(i=>i.value);if($('decoyEnabled').checked&&decoyPools.length<1)throw new Error('启用诱捕需勾选至少 1 个金丝雀池（独立 IP）');if($('decoyEnabled').checked&&!decoySources.length)throw new Error('请勾选至少 1 个嫌疑来源池');if($('decoyEnabled').checked&&decoySources.some(id=>decoyPools.includes(id)))throw new Error('来源池与金丝雀池不能重叠');const body={wall_auto_isolate:$('wallAutoIsolate').checked,wall_hit_threshold:Number($('wallThreshold').value)||2,wall_lookback_seconds:Number($('wallLookback').value)||3600,wall_observe_pool_id:$('wallObservePool').value,decoy_enabled:$('decoyEnabled').checked,decoy_source_pool_ids:decoySources.join(','),decoy_pool_ids:decoyPools.join(','),decoy_confirm_pool_id:$('decoyConfirm').value,decoy_batch_size:Number($('decoyBatch').value)||80,decoy_observe_minutes:Number($('decoyObserve').value)||40,decoy_start:$('decoyStart').value.trim()||'01:00',decoy_end:$('decoyEnd').value.trim()||'09:00'};await request('/wall-settings',{method:'POST',body:JSON.stringify(body)});toast('跟墙 / 诱捕设置已保存');await loadWallLog()}catch(error){toast(error.message,'error')}};
