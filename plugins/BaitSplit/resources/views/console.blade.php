...
<main class="page">
    ...
    <div class="grid">
        ...
        <section class="card wide">
            ...
            <div class="split">
                <div class="field"><label>初筛 / 二验车道每批人数</label><input id="decoyBatch" type="number" min="1" max="5000" value="100"></div>
...
if(document.activeElement!==$('decoyBatch'))$('decoyBatch').value=d.batch_size||100;
...
$('saveWallSettings').onclick=async()=>{try{const decoySources=[...document.querySelectorAll('.decoySource:checked')].map(i=>i.value);if($('decoyEnabled').checked&&!decoySources.length)throw new Error('启用诱捕需勾选至少 1 个嫌疑来源池');const batch=Number($('decoyBatch').value)||100;const body={wall_auto_isolate:$('wallAutoIsolate').checked,wall_hit_threshold:Number($('wallThreshold').value)||2,wall_lookback_seconds:Number($('wallLookback').value)||3600,wall_fresh_max_seconds:Number($('wallFresh').value)||7200,wall_observe_pool_id:$('wallObservePool').value,decoy_enabled:$('decoyEnabled').checked,decoy_monitor_24h:true,decoy_source_pool_ids:decoySources.join(','),decoy_isolate_pool_id:$('decoyIsolate').value,decoy_confirm_pool_id:$('decoyConfirm').value,decoy_batch_size:batch,decoy_hot_batch_size:10,decoy_verify_batch_size:batch,decoy_min_batch:1,decoy_observe_minutes:Number($('decoyObserve').value)||180,decoy_cooldown_minutes:60,decoy_risk_start:'01:00',decoy_risk_end:'09:00',decoy_start:'01:00',decoy_end:'09:00'};await request('/wall-settings',{method:'POST',body:JSON.stringify(body)});toast('三车道精确 IP 监控设置已保存');await loadWallLog()}catch(error){toast(error.message,'error')}};
