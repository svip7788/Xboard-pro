    ...
    <div class="grid">
        ...
        <section class="card wide">
            ...
            <div class="split" style="margin-top:12px">
                <div class="field"><label><input id="wallAutoIsolate" type="checkbox" style="width:auto;height:auto"> 达阈值自动隔离到观察3</label></div>
                <div class="field"><label>隔离目标池 ID（留空自动选“观察3”）</label><input id="wallObservePool" placeholder="留空自动"></div>
            </div>
            <div class="split">
            ...
            <h3 style="margin-top:20px">夜间分组诱捕（主动揪内鬼）<span id="decoyState" class="pill off" style="margin-left:8px">未启用</span></h3>
            <div class="hint">墙时段把「嫌疑来源池」的人随机分到多个各自独立 IP 的组；哪组 IP 被墙内鬼就在那组，累计达阈值自动确认并移入危险组，白天自动收回恢复使用。至少勾选 2 个独立 IP 的分组池。</div>
            <div class="split" style="margin-top:12px">
            ...
            <div class="split">
                <div class="field"><label>嫌疑来源池（每晚从这里抽人打散）</label><select id="decoySource"></select></div>
                <div class="field"><label>确认内鬼隔离池</label><select id="decoyConfirm"></select></div>
            </div>
            <div class="field"><label>分组池（勾选 ≥2 个各自独立 IP 的池）</label><div id="decoyPools" class="group-list"></div></div>

            <button id="saveWallSettings">保存跟墙 / 诱捕设置</button>

            <h3 style="margin-top:20px">今晚分组情况</h3>
...
renderDecoy()}
function renderDecoy(){const d=wallData?.decoy;if(!d)return;const st=$('decoyState');if(d.enabled){if(d.active_date){st.textContent='今晚诱捕中';st.className='pill on'}else if(d.in_window){st.textContent='时段内·待分组';st.className='pill warn'}else{st.textContent='已启用·白天';st.className='pill off'}}else{st.textContent='未启用';st.className='pill off'}
...
const all=pools();
fillSelect('decoySource',all,d.raw?.source||'','自动（观察3）');
...
const g=$('decoyGroups');g.textContent='';(d.groups||[]).forEach(gr=>{const card=document.createElement('div');card.className='pool';const head=document.createElement('div');head.className='pool-head';const t=document.createElement('strong');t.textContent=gr.pool_name;const s=document.createElement('span');s.className=`pill ${gr.walled?'bad':'on'}`;s.textContent=gr.walled?'已被墙':'观察中';head.append(t,s);const meta=document.createElement('div');meta.className='meta';meta.textContent=`${gr.host||'无IP'} · ${gr.count} 人`;card.append(head,meta);g.appendChild(card)});if(!(d.groups||[]).length){g.innerHTML='<div class="empty">今晚尚未分组（到诱捕时段自动打散）</div>'}}
...
$('saveWallSettings').onclick=async()=>{try{const decoyPools=[...document.querySelectorAll('.decoyPool:checked')].map(i=>i.value);if($('decoyEnabled').checked&&decoyPools.length<2)throw new Error('启用诱捕需勾选至少 2 个各自独立 IP 的分组池');const body={wall_auto_isolate:$('wallAutoIsolate').checked,wall_hit_threshold:Number($('wallThreshold').value)||2,wall_lookback_seconds:Number($('wallLookback').value)||3600,wall_observe_pool_id:$('wallObservePool').value.trim(),decoy_enabled:$('decoyEnabled').checked,decoy_source_pool_id:$('decoySource').value,decoy_pool_ids:decoyPools.join(','),decoy_confirm_pool_id:$('decoyConfirm').value,decoy_start:$('decoyStart').value.trim()||'01:00',decoy_end:$('decoyEnd').value.trim()||'09:00'};await request('/wall-settings',{method:'POST',body:JSON.stringify(body)});toast('跟墙 / 诱捕设置已保存');await loadWallLog()}catch(error){toast(error.message,'error')}};
