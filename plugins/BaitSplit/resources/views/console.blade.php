...
const g=$('decoyGroups');g.textContent='';(d.domains||[]).forEach(dm=>{const card=document.createElement('div');card.className='pool';const head=document.createElement('div');head.className='pool-head';const t=document.createElement('strong');t.textContent=dm.pool_name;const s=document.createElement('span');const phase=dm.phase||'idle';const testing=(dm.current_batch||0)>0;let label='未触发',cls='off';if(phase==='armed'){label='已武装·再墙即甩';cls='warn'}else if(phase==='testing'){label='旧测试态·将清理';cls='off'}else if(testing){label=`残留在测 ${dm.current_batch}`;cls='off'}s.className=`pill ${cls}`;s.textContent=label;head.append(t,s);const meta=document.createElement('div');meta.className='meta';meta.textContent=`阶段 ${phase} · 武装IP ${dm.armed_ip||'-'} · 当前IP ${dm.cur_ip||'-'}（再墙时拉过武装IP的人进观察3）`;card.append(head,meta);g.appendChild(card)});if(!(d.domains||[]).length){g.innerHTML='<div class="empty">暂无来源池（请勾选嫌疑来源池并启用）</div>'}}
