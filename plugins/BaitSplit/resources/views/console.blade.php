...
$('closeTransfer').onclick=()=>{$('transferModal').classList.remove('show');transferSource=null};$('transferModal').onclick=event=>{if(event.target===$('transferModal'))$('closeTransfer').click()};$('confirmTransfer').onclick=async()=>{if(!transferSource)return;const source=transferSource,targetId=$('transferTarget').value,targetName=$('transferTarget').options[$('transferTarget').selectedIndex]?.textContent,isTree=source.mode?.startsWith('tree-'),moveUnpulled=source.mode==='tree-unpulled',userType=moveUnpulled?'未拉取':'已拉取';if(!confirm(`把“${source.name}”${userType}用户转入“${targetName}”？${isTree?'\n迁移用户将自动锁定，其他用户继续留在当前分支。':''}`))return;try{const path=!isTree?`/pools/${encodeURIComponent(source.id)}/move-pulled`:moveUnpulled?`/investigations/${encodeURIComponent(source.id)}/move-unpulled`:`/investigations/${encodeURIComponent(source.id)}/move`;const result=await request(api(path),{method:'POST',body:JSON.stringify({target_pool_id:targetId})});updateCurrent(result.campaign);$('closeTransfer').click();toast(isTree?`已移动并锁定 ${result.moved_count} 名${userType}用户，原分支剩余 ${result.remaining_count} 人`:`已移动 ${result.moved_count} 人`)}catch(error){toast(error.message,'error')}};
