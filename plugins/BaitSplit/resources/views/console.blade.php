...
<main class="page">
    ...
    <div class="grid">
        ...
        <section class="card wide">
            ...
            <h3 style="margin-top:20px">墙后二分追猎<span id="huntState" class="pill off" style="margin-left:8px">未启用</span></h3>
            <div class="hint">内鬼要上报就必须先刷新订阅拿新 IP，所以每次墙都能得到干净结论：没拉过被墙 IP 的人当轮洗清，拉过的人对半切进两个槽（各配全新 IP），哪个槽再被墙人就在那一半。开关与池子在插件配置里设置。</div>
            <div id="huntAlert"></div>
            <div id="huntChains"></div>

            <h3 style="margin-top:20px">全天精确 IP 诱捕<span id="decoyState" class="pill off" style="margin-left:8px">未启用</span></h3>
            <div class="hint">三车道隔离搜索：A 专门收敛高风险分支，B 大批初筛，C 第二轮阴性确认，三条车道互不借用。墙事件全天记录，但清白观察时长只累计 01:00–09:00 高风险时段。<span id="decoyChain"></span></div>
            <div class="split" style="margin-top:12px">
                <div class="field"><label><input id="decoyEnabled" type="checkbox" style="width:auto;height:auto"> 启用全天自动诱捕（每分钟调度检查）</label></div>
            </div>
            <div class="split">
                <div class="field"><label>B 初筛车道每批人数</label><input id="decoyBatch" type="number" min="1" max="5000" value="300"></div>
                <div class="field"><label>C 二验车道每批人数</label><input id="decoyVerifyBatch" type="number" min="1" max="5000" value="200"></div>
            </div>
            <div class="split">
...
suspects.textContent='';(wallData.top_suspects||[]).forEach(item=>{const row=suspects.insertRow();row.insertCell().textContent=item.user_id;row.insertCell().textContent=item.email||'-';const hitCell=row.insertCell();const sc=Number(item.score||0);hitCell.textContent=sc>0?`${sc.toFixed(2)}分 / ${item.hits}次`:`${item.hits}次`;hitCell.title=`加权嫌疑分 ${sc.toFixed(3)}（每次墙按 1/本批人数 计，小批命中权重高）；跟墙 ${item.hits} 次`;if(sc>=1||item.hits>=(settings.hit_threshold||2))hitCell.innerHTML=`<span class="pill bad">${hitCell.textContent}</span>`;row.insertCell().textContent=formatTime(item.last_at);const cell=row.insertCell(),button=document.createElement('button');button.className='danger small';button.textContent='隔离到观察3';button.onclick=()=>isolateSuspect(item.user_id);cell.appendChild(button)});if(!(wallData.top_suspects||[]).length){const row=suspects.insertRow();row.insertCell().colSpan=5;row.cells[0].className='empty';row.cells[0].textContent='暂无跟墙嫌疑用户'}
events.textContent='';(wallData.events||[]).forEach(ev=>{const row=events.insertRow();const timeCell=row.insertCell();timeCell.textContent=formatTime(ev.at);if(ev.mode==='decoy'){const ph=ev.phase||(ev.pools||[]).find(p=>p&&p.phase)?.phase||'';timeCell.innerHTML+=ph==='armed'?' <span class="pill warn">武装</span>':(ph==='testing'?' <span class="pill on">测</span>':' <span class="pill warn">诱</span>')}if(ev.mode==='hunt'){const hp={walled_without_pullers:['bad','无人拉取'],chain_seeded:['on','开链'],chain_split:['on','切半'],convicted:['bad','定罪'],no_free_chain:['warn','槽满'],all_pullers_busy:['off','已在测'],stale_wall:['off','迟到']}[ev.phase]||['warn','追猎'];timeCell.innerHTML+=` <span class="pill ${hp[0]}">${hp[1]}</span>`}
if(ev.mode==='manual_fix')timeCell.innerHTML+=' <span class="pill warn">补</span>';if((ev.pools||[]).some(p=>p&&p.stale))timeCell.innerHTML+=' <span class="pill off" title="老 IP 首墙，只换 IP 不记分">跳过</span>';const reasonCell=row.insertCell();reasonCell.innerHTML=`<span class="pill ${ev.reason==='blocked'?'bad':'off'}">${wallReasonLabel(ev.reason)}</span>`;row.insertCell().textContent=`${ev.old_ip||'-'} → ${ev.new_ip||'-'}`;row.insertCell().textContent=(ev.pools||[]).map(p=>typeof p==='string'?p:p.pool_name).join('、')||'-';row.insertCell().textContent=`${ev.suspect_count||0} / ${ev.scored_count||0}`;row.insertCell().textContent=ev.moved_count||0});if(!(wallData.events||[]).length){const row=events.insertRow();row.insertCell().colSpan=6;row.cells[0].className='empty';row.cells[0].textContent='暂无换 IP 事件记录'}
renderDecoy();renderHunt()}
function renderHunt(){const h=wallData?.hunt,st=$('huntState'),box=$('huntChains'),alertBox=$('huntAlert');if(!h||!st)return;
if(!h.enabled){st.textContent='未启用';st.className='pill off'}else if(!h.rotate_api_ready){st.textContent='已启用·未配换 IP 接口';st.className='pill bad'}else if(h.active_chains>0){st.textContent=`追猎中 ${h.active_chains}/${h.chain_capacity} 条链`;st.className='pill on'}else{st.textContent=`已启用·待墙触发（服务池 ${(h.service_pool_names||[]).join('、')||'未配'}）`;st.className='pill warn'}
alertBox.textContent='';(h.anomalies||[]).slice(0,3).forEach(a=>{const div=document.createElement('div');div.className='hint';div.innerHTML=`<span class="pill bad">非泄露铁证</span> ${formatTime(a.at)} ${a.pool_id} 的 IP ${a.ip} <strong>无人拉取却被墙</strong>，没人见过它就没人能上报它——这次墙与订阅泄露无关，优先查 IP 段被扫或协议特征。`;alertBox.appendChild(div)});
(h.convicted||[]).slice(0,3).forEach(c=>{const div=document.createElement('div');div.className='hint';div.innerHTML=`<span class="pill bad">已定罪</span> ${formatTime(c.at)} 用户 <strong>${c.user_id}</strong> 从 ${c.seed_size} 人经 ${c.rounds} 轮墙收敛锁定，已移入危险组。`;alertBox.appendChild(div)});
box.textContent='';(h.chains||[]).forEach(c=>{const card=document.createElement('div');card.className='pool';const head=document.createElement('div');head.className='pool-head';const t=document.createElement('strong');t.textContent=`第 ${c.round} 轮 · 种子 ${c.seed_size} 人（来自 ${c.origin_pool_name||'-'}）`;const s=document.createElement('span');s.className='pill on';s.textContent=`最近事件 ${formatTime(c.last_event_at)}`;head.append(t,s);card.appendChild(head);(c.slots||[]).forEach(sl=>{const line=document.createElement('div');line.className='hint';line.textContent=`${sl.pool_name}：${sl.awaiting_ip?`${sl.pending} 人等全新 IP 上线`:`在测 ${sl.members} 人`} · ${sl.host||'无IP'}`;card.appendChild(line)});box.appendChild(card)});
if(h.enabled&&!(h.chains||[]).length){const div=document.createElement('div');div.className='hint';div.textContent=`追猎槽：${(h.slot_pool_names||[]).join('、')||'未配置'}（两两成链，共 ${h.chain_capacity} 条）。等服务池被墙即自动开链。`;box.appendChild(div)}}
function renderDecoy(){const d=wallData?.decoy;if(!d)return;const chain=$('decoyChain');if(chain){const slots=d.search_pool_names||[];if(slots.length)chain.textContent=` 当前车道：高风险 ${d.hot_waiting||0} 人 · 二验 ${d.verify_waiting||0} 人 · 初筛 ${d.bulk_waiting||0} 人；两次阴性进 ${d.candidate_pool_name||'观察1'}，单人两次阳性进危险组（观察1候选 ${d.candidate_users||0} 人，待人工确认 ${d.candidate_ready||0} 人）`;else chain.textContent=(d.isolate_chain_names||[]).length?` 当前链路：${(d.source_pool_names||[]).join('、')} → ${(d.isolate_chain_names||[]).join(' → ')}`:''}const st=$('decoyState');if(d.enabled){if((d.testing_count||0)>0){st.textContent=`${d.monitor_24h?'全天监控':'诱捕中'}·在测 ${d.testing_count} 人`;st.className='pill on'}else if(d.in_window){st.textContent=d.monitor_24h?'全天监控·待墙触发':'时段内·待墙触发';st.className='pill warn'}else{st.textContent='已启用·窗口外';st.className='pill off'}}else{st.textContent='未启用';st.className='pill off'}
if(chain&&(d.fresh_waiting||d.dormant_waiting))chain.textContent+=`；近期优先 ${d.fresh_waiting||0} 人，休眠待拉取 ${d.dormant_waiting||0} 人`;
if(document.activeElement!==$('decoyEnabled'))$('decoyEnabled').checked=!!d.enabled;
if(document.activeElement!==$('decoyBatch'))$('decoyBatch').value=d.batch_size||300;
if(document.activeElement!==$('decoyVerifyBatch'))$('decoyVerifyBatch').value=d.verify_batch_size||200;
...
const g=$('decoyGroups');g.textContent='';const slotIds=new Set((d.search_pool_ids||[]).map(String)),laneNames={hot:'高风险快审',bulk:'大批初筛',verify:'二轮清白确认'};(d.domains||[]).forEach(dm=>{const card=document.createElement('div');card.className='pool';const head=document.createElement('div');head.className='pool-head';const t=document.createElement('strong');t.textContent=dm.pool_name;const s=document.createElement('span');const phase=dm.phase||'idle';const testing=(dm.current_batch||0)>0;const isSlot=slotIds.has(String(dm.pool_id));const grp=Number(dm.group_size||0);const cooling=Number(dm.cooldown_until||0)>Math.floor(Date.now()/1000);let label='未触发',cls='off';if(isSlot){if(grp>0){label=`${laneNames[dm.lane]||'动态车道'} · ${grp} 人`;cls=dm.lane==='hot'?'bad':'on'}else if(cooling){label='搜索槽·冷却中';cls='warn'}else{label='搜索槽·空闲';cls='off'}}else if(phase==='armed'){label='已武装·精确IP监控';cls='warn'}else if(phase==='testing'){label='旧测试态·将清理';cls='off'}else if(testing){label=`残留在测 ${dm.current_batch}`;cls='off'}s.className=`pill ${cls}`;s.textContent=label;head.append(t,s);const meta=document.createElement('div');meta.className='meta';meta.textContent=isSlot?`阶段 ${phase} · 当前IP ${dm.cur_ip||'-'}${grp>0?` · 每人首次拉取后观察 ${d.observe_minutes||180} 分钟（仅累计 01:00–09:00）`:cooling?' · 防迟到墙冷却中':' · 等待动态派发'}`:`阶段 ${phase} · 武装IP ${dm.armed_ip||'-'} · 当前IP ${dm.cur_ip||'-'}（再墙只处理实际拉过该精确IP的人）`;card.append(head,meta);g.appendChild(card)});if(!(d.domains||[]).length){g.innerHTML='<div class="empty">暂无监控池（等待首次墙事件武装）</div>'}}
...
$('saveWallSettings').onclick=async()=>{try{const decoySources=[...document.querySelectorAll('.decoySource:checked')].map(i=>i.value);if($('decoyEnabled').checked&&!decoySources.length)throw new Error('启用诱捕需勾选至少 1 个嫌疑来源池');const batch=Number($('decoyBatch').value)||300,verifyBatch=Number($('decoyVerifyBatch').value)||200;const body={wall_auto_isolate:$('wallAutoIsolate').checked,wall_hit_threshold:Number($('wallThreshold').value)||2,wall_lookback_seconds:Number($('wallLookback').value)||3600,wall_fresh_max_seconds:Number($('wallFresh').value)||7200,wall_observe_pool_id:$('wallObservePool').value,decoy_enabled:$('decoyEnabled').checked,decoy_monitor_24h:true,decoy_source_pool_ids:decoySources.join(','),decoy_isolate_pool_id:$('decoyIsolate').value,decoy_confirm_pool_id:$('decoyConfirm').value,decoy_batch_size:batch,decoy_hot_batch_size:10,decoy_verify_batch_size:verifyBatch,decoy_min_batch:1,decoy_observe_minutes:Number($('decoyObserve').value)||180,decoy_cooldown_minutes:60,decoy_risk_start:'01:00',decoy_risk_end:'09:00',decoy_start:'01:00',decoy_end:'09:00'};await request('/wall-settings',{method:'POST',body:JSON.stringify(body)});toast('三车道精确 IP 监控设置已保存');await loadWallLog()}catch(error){toast(error.message,'error')}};
