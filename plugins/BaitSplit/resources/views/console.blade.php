...
<main class="page">
    ...
    <div class="grid">
        ...
        <section class="card wide">
            ...
            <div class="hint">勾选一个或多个最上层根组：一棵可重新打乱，多棵可合并；仅处理已拉取用户，未拉取用户回到默认容量链。</div>
...
function openMergeTree(){const nodes=router()?.investigation_nodes||[],selected=nodes.filter(node=>mergeTreeNodeIds.has(node.id));if(!selected.length)return toast('请至少选择一个最上层根组','error');$('mergeTreeHint').textContent=`已选择 ${selected.length} 棵旧树，共 ${selected.reduce((sum,node)=>sum+treeMergeableCount(node.id,nodes),0)} 名已拉取用户。未拉取用户将回到默认容量链。`;$('mergeTreeName').value=selected.length>1?'合并排查树':'重组排查树';$('mergeTreeCount').value=2;renderMergeTreeFields();$('mergeTreeModal').classList.add('show')}
