...
<head>
    ...
    <style>
        ...
        *{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--text);font:14px/1.6 -apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif}
        ...
        /* 展开收起 */
        .tree-toggle{display:inline-flex;align-items:center;justify-content:center;width:24px;height:24px;margin-right:8px;background:#e0e7ff;border:none;border-radius:6px;cursor:pointer;font-size:12px;transition:transform .2s,background .15s}
...
<main class="page">
    ...
    <div id="usersModal" class="modal"><div class="modal-card">
        ...
        <div id="poolUserTools" class="modal-tools" style="display:none"><input id="poolUserSearch" placeholder="搜索用户 ID 或邮箱"><button id="searchPoolUsers">搜索</button><button id="filterPulled" class="secondary">已拉取</button><button id="filterUnpulled" class="secondary">未拉取</button><button id="filterAll" class="secondary">全部</button></div>
    ...
    <div id="startInvestigationModal" class="modal"><div class="modal-card" style="width:min(520px,100%)">
        ...
        <input id="startInvestigationPoolId" type="hidden">
        <p id="startInvestigationHint" class="hint"></p>
        <div class="field"><label>排查树名称（可留空）</label><input id="startInvestigationName" placeholder="默认使用池子名称"></div>
        <div class="field"><label><input id="startInvestigationOnlyExposed" type="checkbox" style="width:auto;height:auto"> 只选择已拉取用户（未拉取用户留在原组）</label></div>
        <button id="confirmStartInvestigation">确认进入排查</button>
    </div></div>
    <div id="splitTreeModal" class="modal"><div class="modal-card" style="width:min(680px,100%)">
        ...
        <p id="splitTreeHint" class="hint"></p>
        <div class="field"><label><input id="splitTreeOnlyExposed" type="checkbox" style="width:auto;height:auto"> 只细分已拉取用户（未拉取用户保留在当前节点）</label></div>
    ...
    <div class="grid">
        ...
        <section class="card wide">
            <div class="topbar"><h2>树形分支排查</h2><div class="actions"><button id="toggleAllTrees" class="secondary small">全部展开</button><button id="openMergeTree" class="warning" disabled>合并旧排查树（0）</button></div></div>
        ...
        <section class="card wide">
            <div class="topbar">
                <h2>换 IP 事件日志</h2>
                <div class="actions">
                    <label class="hint" style="display:flex;align-items:center;gap:6px"><input id="autoResetOnWall" type="checkbox" style="width:16px;height:16px"> 墙后重置token/uuid</label>
                    <span id="wallPending" class="pill off">换IP队列 0</span>
                    <button id="refreshWall" class="secondary">刷新</button>
                </div>
            </div>
            <div class="wall-tools" style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin-bottom:12px">
                ...
                <select id="wallPoolFilter" style="width:160px;height:34px"><option value="">全部池</option></select>
...
function getTransferTargets(excludePoolId=''){const poolTargets=pools().filter(item=>item.id!==excludePoolId&&item.enabled&&item.status!=='blocked').map(p=>({id:p.id,name:`[池] ${p.name}`,type:'pool'}));const nodes=router()?.investigation_nodes||[];const treeTargets=nodes.filter(n=>n.pool_id!==excludePoolId&&!n.children.length&&!['blocked','archived'].includes(n.status)).map(n=>({id:n.pool_id,name:`[树] ${n.name}`,type:'tree',nodeId:n.id}));return [...poolTargets,...treeTargets]}
...
function serverUsedByOtherTask(serverId){const selectedGroups=new Set(selectedGroupIds().map(Number));return campaigns.find(item=>item.id!==current?.id&&(item.target_group_ids||[item.target_group_id]).some(id=>selectedGroups.has(Number(id)))&&(item.target_server_ids||[]).some(id=>Number(id)===Number(serverId)))}
function renderNodes(){const box=$('nodeSelect'),list=groupServers();box.textContent='';if(!list.length){box.innerHTML='<div class="empty">请选择用户组后显示节点</div>';return}const excluded=new Set((current?.excluded_server_ids||[]).map(Number));list.forEach(server=>{const usedBy=serverUsedByOtherTask(server.id);const label=document.createElement('label');label.className='node';if(usedBy)label.style.opacity='.55';const input=document.createElement('input');input.type='checkbox';input.className='nodeReplace';input.value=server.id;input.disabled=!!usedBy;input.checked=!usedBy&&!excluded.has(Number(server.id));const name=document.createElement('span');name.textContent=server.name;const tag=document.createElement('small');tag.textContent=usedBy?`${server.type} · 已在 ${usedBy.name}`:server.type;label.title=usedBy?`该节点已属于任务：${usedBy.name}`:'';label.append(input,name,tag);box.appendChild(label)})}
...
function getTransferTargets(excludePoolId=''){const poolTargets=pools().filter(item=>item.id!==excludePoolId&&item.enabled&&item.status!=='blocked').map(p=>({id:p.id,name:`[池] ${p.name}`,type:'pool'}));const nodes=router()?.investigation_nodes||[];const treeTargets=nodes.filter(n=>n.pool_id!==excludePoolId&&!n.children.length&&!['blocked','archived'].includes(n.status)).map(n=>({id:n.pool_id,name:`[树] ${n.name}`,type:'tree',nodeId:n.id}));return [...poolTargets,...treeTargets]}
function openPoolTransfer(pool,movePulled=true){const targets=getTransferTargets(pool.id);if(!targets.length)return toast('暂无可用的转入目标','error');const unpulledCount=Math.max(0,pool.member_count-pool.pulled_count);transferSource={...pool,mode:movePulled?'pool-pulled':'pool-unpulled'};fillSelect('transferTarget',targets,'');$('transferTitle').textContent=movePulled?'转移已拉取用户':'转移未拉取用户';$('transferHint').textContent=movePulled?`"${pool.name}"当前有 ${pool.pulled_count} 名已拉取用户（含锁定），转移后会继续锁定。`:`"${pool.name}"当前有 ${unpulledCount} 名未拉取用户（含锁定），转移后会继续锁定；已拉取的 ${pool.pulled_count} 人留在当前组。`;$('transferModal').classList.add('show')}
function openTreeTransfer(node,movePulled=true){const targets=getTransferTargets(node.pool_id);if(!targets.length)return toast('暂无可用的转入目标','error');const unpulledCount=Math.max(0,node.user_count-node.pulled_count);transferSource={...node,mode:movePulled?'tree-pulled':'tree-unpulled'};fillSelect('transferTarget',targets,'');$('transferTitle').textContent=movePulled?'转移已拉取用户':'转移未拉取用户';$('transferHint').textContent=movePulled?`"${node.name}"已有 ${node.pulled_count} 人拉取（含锁定）；只迁移这些已拉取用户并锁定，剩余 ${unpulledCount} 人继续留在当前分支观察。`:`"${node.name}"有 ${unpulledCount} 人尚未拉取（含锁定）；只迁移这些未拉取用户并锁定，已拉取的 ${node.pulled_count} 人继续留在当前分支。`;$('transferModal').classList.add('show')}
function openStartInvestigation(pool){$('startInvestigationPoolId').value=pool.id;$('startInvestigationName').value='';$('startInvestigationOnlyExposed').checked=false;$('startInvestigationHint').textContent=`"${pool.name}"当前有 ${pool.member_count} 人，其中 ${pool.pulled_count} 人已拉取。进入排查后用户会冻结到独立树中，需继续拆分并填写新域名。`;$('startInvestigationModal').classList.add('show')}
async function poolAction(action,pool){try{if(action==='copy-id'){const targetId=pool.webhook_id||pool.id;await navigator.clipboard.writeText(targetId);return toast(`接口标识已复制：${targetId}`)}if(action==='edit')return editPool(pool);if(action==='users')return showPoolUsers(pool);if(action==='transfer')return openPoolTransfer(pool,true);if(action==='transfer-unpulled')return openPoolTransfer(pool,false);if(action==='tree'){return openStartInvestigation(pool)}if(action==='delete'){if(!confirm(`删除“${pool.name}”？`))return;updateCurrent(await request(api(`/pools/${encodeURIComponent(pool.id)}`),{method:'DELETE'}));toast('用户池已删除')}}catch(error){toast(error.message,'error')}}
const treeCollapsed=new Set(JSON.parse(localStorage.getItem('baitSplitTreeCollapsed')||'[]'));
function toggleTreeCollapse(nodeId){if(treeCollapsed.has(nodeId))treeCollapsed.delete(nodeId);else treeCollapsed.add(nodeId);localStorage.setItem('baitSplitTreeCollapsed',JSON.stringify([...treeCollapsed]));renderInvestigationTree()}
function renderInvestigationTree(){
    ...
    // 构建祖先链，用于判断节点是否因祖先收起而隐藏
    const getAncestors=(nodeId)=>{const ancestors=[];let current=byId.get(nodeId);while(current&&current.parent_id){ancestors.push(current.parent_id);current=byId.get(current.parent_id)}return ancestors};
    ordered.forEach(node=>{
        ...
        if(node.children.length>0){
            ...
            toggle.innerHTML='▼';
            toggle.title=isCollapsed?'展开子节点':'收起子节点';
            toggle.onclick=e=>{e.stopPropagation();toggleTreeCollapse(node.id)};
        ...
        if(node.status!=='archived')add(node.depth===0?'删除排查树':'删除分支',()=>deleteInvestigationTree(node),'danger');
        card.append(head,host,metaLine,actions);
        list.appendChild(card);
    });
    updateMergeTreeButton()
}
async function deleteInvestigationTree(node){const isRoot=node.depth===0;const msg=isRoot?`删除"${node.name}"整棵排查树及全部下级分支？`:`删除"${node.name}"分支及其下级？`;if(!confirm(`${msg}
仍在该分支的用户会解除固定分组，下次拉订阅时重新分配。手动锁定规则不受影响。`))return;try{const result=await request(api(`/investigations/${encodeURIComponent(node.id)}`),{method:'DELETE'});updateCurrent(result.campaign);toast(`${isRoot?'排查树':'分支'}已删除，${result.released_count} 名用户等待重新分组`)}catch(error){toast(error.message,'error')}}
...
function openSplitTree(node){splitTreeNodeId=node.id;$('splitTreeTitle').textContent=`拆分：${node.name}（${node.user_count} 人）`;$('splitTreeHint').textContent=`该节点有 ${node.user_count} 人，其中 ${node.pulled_count} 人已拉取。`;$('splitTreeOnlyExposed').checked=false;$('splitTreeCount').value=2;renderSplitTreeFields();$('splitTreeModal').classList.add('show')}
...
$('autoResetOnWall').onchange=async()=>{const enabled=$('autoResetOnWall').checked;if(enabled&&!confirm('开启后，每次真实被墙都会重置拿到过旧 IP 的用户 token 和 uuid，确认开启？')){$('autoResetOnWall').checked=false;return}try{loading(true,'正在保存开关…');updateCurrent(await request(api('/auto-reset-on-wall'),{method:'POST',body:JSON.stringify({enabled})}));toast(enabled?'已开启墙后自动重置':'已关闭墙后自动重置')}catch(error){toast(error.message,'error');renderCurrent()}finally{loading(false)}};
// 单条日志转移用户
...
$('filterPulled').onclick=()=>reloadPagedUsersWithFilter('pulled');$('filterUnpulled').onclick=()=>reloadPagedUsersWithFilter('unpulled');$('filterAll').onclick=()=>reloadPagedUsersWithFilter('');
$('closeStartInvestigation').onclick=()=>$('startInvestigationModal').classList.remove('show');$('startInvestigationModal').onclick=event=>{if(event.target===$('startInvestigationModal'))$('closeStartInvestigation').click()};$('confirmStartInvestigation').onclick=async()=>{const poolId=$('startInvestigationPoolId').value,name=$('startInvestigationName').value.trim(),onlyExposed=$('startInvestigationOnlyExposed').checked;if(!poolId)return;try{updateCurrent(await request(api(`/pools/${encodeURIComponent(poolId)}/investigation`),{method:'POST',body:JSON.stringify({name,only_exposed:onlyExposed})}));$('closeStartInvestigation').click();toast('已创建排查根组，请继续拆分');document.getElementById('investigationTree').scrollIntoView({behavior:'smooth'})}catch(error){toast(error.message,'error')}};
...
$('splitTreeCount').oninput=renderSplitTreeFields;$('closeSplitTree').onclick=()=>{$('splitTreeModal').classList.remove('show');splitTreeNodeId=''};$('splitTreeModal').onclick=event=>{if(event.target===$('splitTreeModal'))$('closeSplitTree').click()};$('confirmSplitTree').onclick=async()=>{if(!splitTreeNodeId)return;const branches=branchValues('splitTreeBranches'),onlyExposed=$('splitTreeOnlyExposed').checked;if(branches.some(branch=>!branch.host))return toast('请填写每个分支的全新域名','error');const confirmMsg=onlyExposed?`确认只把已拉取用户均分为 ${branches.length} 个独立分支？未拉取用户将保留在当前节点。`:`确认把该节点固定均分为 ${branches.length} 个独立分支？`;if(!confirm(confirmMsg))return;try{updateCurrent(await request(api(`/investigations/${encodeURIComponent(splitTreeNodeId)}/split`),{method:'POST',body:JSON.stringify({branches,only_exposed:onlyExposed})}));$('closeSplitTree').click();toast(onlyExposed?'下级分支已创建，只有已拉取用户被分配':'下级分支已创建，用户分配已固定')}catch(error){toast(error.message,'error')}};
$('toggleAllTrees').onclick=()=>{const nodes=router()?.investigation_nodes||[];const parentIds=nodes.filter(n=>n.children.length>0).map(n=>n.id);const allCollapsed=parentIds.length>0&&parentIds.every(id=>treeCollapsed.has(id));if(allCollapsed){parentIds.forEach(id=>treeCollapsed.delete(id));$('toggleAllTrees').textContent='全部收起'}else{parentIds.forEach(id=>treeCollapsed.add(id));$('toggleAllTrees').textContent='全部展开'}localStorage.setItem('baitSplitTreeCollapsed',JSON.stringify([...treeCollapsed]));renderInvestigationTree()};
