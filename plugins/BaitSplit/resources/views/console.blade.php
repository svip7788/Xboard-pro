...
const note=$('convergeNote');if(note){if(!nc||!nc.enabled){note.textContent=''}else{const total=Object.values(counts).reduce((a,b)=>a+b,0);note.textContent=`窗口内共收敛 ${total} 人：${nc.risk_uid_count?`高危名单 ${nc.risk_uid_count} 人全压第一个牺牲池，其余进第二个`:'名单为空，按 uid 均分'}；${nc.members_only?'只收归属牺牲池的人，其他组保持原 IP':'全站收敛，其他组的人也会被塞进牺牲池'}`}}}
